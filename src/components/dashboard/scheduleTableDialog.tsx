...
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuGroup, DropdownMenuTrigger } from '../ui/dropdown-menu';
...
}) 
{
  ...
  const [updatedScheduleSlots, setUpdatedScheduleSlots] = useState<(ScheduleSlotRequest)[] >([])
  ...
  const [everyDayChecked, setEveryDayChecked] = useState<boolean | 'indeterminate'>('indeterminate')
  const addButtonDisabled = selectedDays.length < 1 || selectedStartTime === '' || selectedEndTime === ''
  ...
  const addScheduleSlot = () => {
    ...
    selectedDays.forEach((day) => {
      ...
      if (updatedScheduleSlots.find((slot) => slot.id === id) === undefined) {
        ...
        const selectedStartHourStr = twelveHourClock ? selectedStartHour.twelveHourStr : selectedStartHour.absHourStr
        const selectedEndHourStr = twelveHourClock ? selectedEndHour.twelveHourStr : selectedEndHour.absHourStr
        toast.error(`Duplicate(s) detected. ${day}, (${selectedStartHourStr} - ${selectedEndHourStr}) already in schedule.`)
      }
    })
    
    setUpdatedScheduleSlots(updatedScheduleSlots.concat(newScheduleSlots).sort((a,b) => a.rank - b.rank))
    clearInputFields();
  }

  function scheduleSlotsAreEqual(slotsA: ScheduleSlotRequest[], slotsB: ScheduleSlotRequest[]): boolean {
    if (slotsA.length !== slotsB.length) return false
    let aStr = ""
    let bStr = ""
    slotsA.sort((a, b) => a.rank - b.rank).forEach((a) => {
      aStr = aStr.concat(a.id)
    })
    slotsB.sort((a, b) => a.rank - b.rank).forEach((b) => {
  ...
  const getPossibleEndTimes = (): HourOfDay[] => {
    const selectedHourOfDay = selectedStartTime ? getSelectedHourOfDay(selectedStartTime) : hoursInDay[0]
  ...
  const removeSlot = (slot: ScheduleSlotRequest) => {
    ...
    const scheduleSlotsModified = !scheduleSlotsAreEqual(oldScheduleSlots, newSlots)
  ...
  function getFormattedTimeSlot(slot: ScheduleSlotRequest): string {
    ...
    const startHourStr = !twelveHourClock ? startHourOfDay?.absHourStr : startHourOfDay?.twelveHourStr 
    const endHourStr = !twelveHourClock ? endHourOfDay?.absHourStr : endHourOfDay?.twelveHourStr 
  ...
  return(
    <DialogContent className="lg:max-w-fit sm:max-w-[425px] bg-secondary">
          ...
          {updatedScheduleSlots.length > 0 && (
            <ItemGroup className="grid min-h-full lg:grid-cols-2 sm:grid-cols-1 gap-2 lg:max-w-fit py-2">
              { updatedScheduleSlots?.map((slot) => (
                  <Item key={slot.id} variant="outline" size="sm" className="border-2 p-1 min-w-fit ">
                    <ItemContent className="flex flex-row">
                      ...
                      <div className="flex flex-col justify-center w-full text-center grow p-2 text-primary-foreground font-semibold font-mono">{getFormattedTimeSlot(slot)}</div>
                    </ItemContent>
                    <ItemActions>
        ...
        <div className="flex flex-row gap-3 w-full justify-evenly">
          <div className="flex flex-col gap-3">
            <div className="flex flex-row h-full w-full justify-center gap-3">
              ...
              >
                <DropdownMenuTrigger asChild className="min-w-45 lg:max-w-fit sm:max-w-[150px]">
                  <Button variant="outline" role="combobox" className="justify-between px-3">
                    <span className={`lg:max-w-full sm:max-w-[150px] ${selectedDays.length > 0 ? "text-primary-foreground" : "text-muted-foreground"} text-sm truncate`}>
                      {getSelectedDaysString()}
                    </span>
                    <ChevronDown className="h-4 w-4 opacity-50" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="lg:max-w-full sm:max-w-[150px]">
                  <DropdownMenuGroup>
                    {daysOfWeek.map((day) => (
            ...
            <div className="flex justify-center gap">
              <Select disabled={isLoadingAvailability} value={selectedStartTime} onValueChange={ (value) => setSelectedStartTime(value) }>
                <SelectTrigger className="max-w-fit">
                  <SelectValue placeholder="From" />
                </SelectTrigger>
                <SelectContent position="item-aligned" className="max-h-[325px] overflow-y-auto">
                  <SelectGroup>
                    {hoursInDay.map((hour, i) => (
                      <SelectItem key={i} value={hour.hour.toString()}>{getFormattedHour(hour)}</SelectItem>                
                    ))}
                  </SelectGroup>
                </SelectContent>
              </Select>
              <span className="mx-1 flex flex-col h-full justify-center"><Minus /></span>
              <Select disabled={isLoadingAvailability} value={selectedEndTime} onValueChange={ (value) => setSelectedEndTime(value) }>
                <SelectTrigger className="max-w-fit">
                  <SelectValue placeholder="Until" />
                </SelectTrigger>
                  <SelectContent position="item-aligned" className="max-h-[325px] max-w-fit overflow-y-auto">
                  <SelectGroup>
          ...
          <div className="flex flex-col h-full w-fit justify-center">
            <div className="flex flex-row items-start justify-center">
              <Checkbox onCheckedChange={checkEveryDay} id="toggle" className="h-8 w-8 border-secondary-foreground" />
            </div>
            <Label htmlFor="toggle" className="text-xs">Every day?</Label>
          </div>
        </div>
        
        <div className="flex flex-col h-full w-fit justify-center">
          <Button onClick={addScheduleSlot} disabled={isLoadingAvailability || addButtonDisabled}>
            Add
            <PlusIcon />
          </Button>
        </div>
      ...
      <DialogFooter className="flex gap-2">
        <DialogClose asChild>
          <Button variant="outline" onClick={resetDialog} disabled={isSendingToApi}>Cancel</Button>
        </DialogClose>
        <Button type="submit" onClick={save} disabled={isSendingToApi || !scheduleModified || isLoadingAvailability}>
          Save
