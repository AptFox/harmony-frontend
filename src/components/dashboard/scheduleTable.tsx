...
function isTimeOff(timeOffSlots: TimeOff[] | undefined, dayOfWeekToDatesMap: Map<string, Date>, day: string, hourOfDay: HourOfDay): boolean {
...
export default function ScheduleTable({ twelveHourClock } : { twelveHourClock: boolean} ){
  ...
  function setHourStatusInMap(map: Map<HourOfDay, Map<string, HourStatus>>, dayOfWeek: string, startHour: number, endHour: number){
    const filterFn = (hourOfDay: HourOfDay) => {
      const hour = hourOfDay.hour;
      return hour >= startHour && hour <= endHour;
    }
    hoursInDay.filter(hourOfDay => filterFn(hourOfDay)).forEach((hourOfDay) => {
      const hourStatus = map.get(hourOfDay)?.get(dayOfWeek) || { isAvailable: false, isTimeOff: false }
      hourStatus.isAvailable = true
      hourStatus.isTimeOff = isTimeOff(timeOffSlots, dayOfWeekToDatesMap, dayOfWeek, hourOfDay)
  ...
  function setAvailabilityInMap(map: Map<HourOfDay, Map<string, HourStatus>>) {
    if (scheduleSlots !== undefined) {
      daysOfWeek.forEach((day) => {
        const slotsForDay = scheduleSlots.filter(slot => slot.dayOfWeek === day);
